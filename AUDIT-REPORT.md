...
| # | Issue | Location | Fix |
|---|---|---|---|
| M1 | **Performance: LCP 4.7–8.0s, FCP ~4.0s, perf 65–73.** ① Non-next-gen images — hero/cab PNGs from `images.ctfassets.net` + Unsplash (~532 KB, +2470 ms); ② render-blocking Google Fonts CSS (~862 ms); ③ heavy main bundle 385.8 KB (124 KB gzip) + booking chunk 106.8 KB (32.6 KB gzip), ~42–63 KB unused JS | `src/components/shared/Hero.tsx`, `CarImage.tsx`, `CoverageMapSection.tsx`; `index.html` fonts; vite build | ① ctfassets serves WebP/AVIF on demand (`?fm=webp` / `&w=…`) + `srcSet`; add `sizes`; lazy-load below-fold; ② `preconnect` fonts.gstatic + trim to 2 families / fewer weights; ③ preload LCP asset, code-split `react-hook-form`/`zod` out of the main chunk (they only serve booking). Target: LCP < 2.5s. |
...
| L10 | City/Service not-found branches self-canonical the bad path (e.g. `/cities/nonsense` canonical → itself), while Route/Article/Page branches canonical to `/` | `src/pages/CityPage.tsx`, `ServicePage.tsx` | Point canonical to `/` like the other 404 branches. |
